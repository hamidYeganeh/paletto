...
const Highlight = React.forwardRef<HTMLDivElement, HighlightProps<any>>(
  (props, forwardedRef) => {
    const {
      as: Component = "div",
      children,
    ...
    const render = (childrenNode: React.ReactNode) => {
      if (mode === "parent") {
        return (
          <Component
            ref={localRef}
...
const HighlightItem = React.forwardRef<HTMLDivElement, HighlightItemProps<any>>(
  (
    {
      as,
      children,
    ...
    const childValue =
      id ??
      value ??
      (element.props as ExtendedChildProps)["data-value"] ??
      element.props?.id ??
      itemId;
